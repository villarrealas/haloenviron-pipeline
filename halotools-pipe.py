...
fixbins = 20        # number of bins for normalization
cores = 1           # number of cores for multiprocessing
vhost_min = 400.0   # minimum vmax for host in satellite counting
...
nrand = 3          # number of randomizations for error bars
...
matching_data = reader.read_ascii()
...
c_matching = coord.SkyCoord(x=matching_data['halo_x'], y=matching_data['halo_y'], z=matching_data['halo_z'],
                            unit='Mpc', frame='icrs', representation='cartesian')

matchid, _, sep3d = coord.match_coordinates_3d(c_matching, c_master)
mask = (sep3d.value <= matching_data['halo_rvir']*.001*.01)
...
# as above but for the entire matching catalog
vratio_temp_matching = matching_data['halo_vmax']/(np.sqrt(gnewton * matching_data['halo_mass']/matching_data['halo_rvir']))
cnfw_temp_matching = matching_data['halo_rvir']/matching_data['halo_rs']
nflagtemp_matching = np.zeros(len(matching_data))
subcount_matching = np.zeros(len(matching_data))

for i in range(0, len(matching_data)):
   if matching_data[i]['halo_vmax'] > vhost_min:
      nflagtemp_matching[i] = 1
      hosts_data_cut = matching_data[np.where(matching_data['halo_pid'] == matching_data[i]['halo_id'])]
      for j in range(0, len(hosts_data_cut)):
         xdist = hosts_data_cut[j]['halo_x'] - matching_data[i]['halo_x']
         ydist = hosts_data_cut[j]['halo_y'] - matching_data[i]['halo_y']
         zdist = hosts_data_cut[j]['halo_z'] - matching_data[i]['halo_z']
         totdist = (np.sqrt(xdist**2 + ydist**2 + zdist**2))*0.001
         if totdist < matching_data[i]['halo_rvir']:
            ratio = hosts_data_cut[j]['halo_vmax']/matching_data[i]['halo_vmax']
            if ratio > vrat_frac:
               subcount_matching[i] = subcount_matching[i] + 1
...
hosts_data_temp = append_fields(matching_data, ('halo_cV', 'halo_cNFW', 'halo_satflag', 'halo_nsat'), (vratio_temp_matching, cnfw_temp_matching, nflagtemp_matching, subcount_matching))

hosts_data_matched = hosts_data_temp[np.where(np.logical_and(mask==True, hosts_data_temp['halo_mass'] >= mthresh))]
print('Matches accepted:', len(hosts_data_matched))
...
hosts_data_sat = hosts_data_alt[np.where(hosts_data_alt['halo_satflag']==1)]
pos_sat = np.vstack((hosts_data_sat['halo_x'], hosts_data_sat['halo_y'], hosts_data_sat['halo_z'])).T
...
# these unfortunately return ALL pairs under a certain definition. Thankfully, we can fix this with np.diff
num_in_bin = np.diff(mo.npairs_3d(pos, pos, rbins=10**logbins, period=lbox))
num_in_bin_sat = np.diff(mo.npairs_3d(pos_sat, pos_sat, rbins=10**logbins, period=lbox))

print len(num_in_bin), num_in_bin
...
sampall_sat = np.random.uniform(0,1,(len(hosts_data_alt[np.where(hosts_data_alt['halo_satflag']==1)]),nrand))
...
#######################################################
# saved for posterity, cut for now for speed purposes #
#######################################################
# calculations as above, but for different marks
...
print len(xi), xi
print len(mcfn_vratio), mcfn_vratio
print len(uppererr), uppererr

np.savetxt(outname, np.transpose([10**binmids, xi, error, xi_cnfw_low, error_cnfw_low, xi_cnfw_high, error_cnfw_high,
                                 mcfn_vratio, mcfn_cnfw, mcfn_ctoa, mcfn_spin, lowererr, uppererr, mcfn_nsat, lowererr_sat,
                                 uppererr_sat]), header=r'[0]r [1]xi [2]xi_err [3]xi_cnfw_low [4]lower_err [5]xi_cnfw_high [6]high_err [7]mcf_cratio [8]mcf_cnfw [9]mcf_ctoa [10]mcf_spin [11]lower err [12]upper err [13]mcf_sat [14]lower mcf err [15]upper mcf err')

print "Main Run Complete -> Beginning Matched Catalog Analysis"


######################################
# OLD FIXING PROTOCOL - AGAIN REMOVED#
######################################
...
X1 = np.log10(hosts_data_matched['halo_mass'])
bins = np.linspace(X1.min(),X1.max(), fixbins)
idx = np.digitize(X1,bins)
for i in range(1, fixbins+1):
   ranks = np.zeros(len(hosts_data_matched[idx==i]))
   ranks = stats.rankdata(hosts_data_matched['halo_cV'][idx==i],'average')
   rank_fix = ranks / len(ranks)
   hosts_data_matched['halo_cV'][idx==i]=rank_fix

   ranks = stats.rankdata(hosts_data_matched['halo_cNFW'][idx==i],'average')
   rank_fix = ranks / len(ranks)
   hosts_data_matched['halo_cNFW'][idx==i]=rank_fix

   ranks = stats.rankdata(hosts_data_matched['halo_ctoa'][idx==i],'average')
   rank_fix = ranks / len(ranks)
   hosts_data_matched['halo_ctoa'][idx==i]=rank_fix

X1 = np.log10(hosts_data_matched['halo_mass'][np.where(hosts_data_matched['halo_satflag']==1)])
bins = np.linspace(X1.min(),X1.max(), fixbins)
idx = np.digitize(X1,bins)

for i in range(1, fixbins+1):
   ranks = np.zeros(len(hosts_data_matched[np.where(hosts_data_matched['halo_satflag']==1)][idx==i]))
   ranks = stats.rankdata(hosts_data_matched[np.where(hosts_data_matched['halo_satflag']==1)][idx==i])
   rank_fix = ranks / len(ranks)
   hosts_data_matched['halo_satflag'][np.where(hosts_data_matched['halo_satflag']==1)][idx==i]=rank_fix

pos = np.vstack((hosts_data_matched['halo_x'], hosts_data_matched['halo_y'], hosts_data_matched['halo_z'])).T
hosts_data_matched_sat = hosts_data_matched[np.where(hosts_data_matched['halo_satflag']==1)]
pos_sat = np.vstack((hosts_data_matched_sat['halo_x'], hosts_data_matched_sat['halo_y'], hosts_data_matched_sat['halo_z'])).T
...
mcf_vratio = mo.marked_tpcf(pos, 10**logbins, marks1=hosts_data_matched['halo_cV'], period=lbox,
                            normalize_by='number_counts', weight_func_id=1, num_threads=cores)
mcfn_vratio = (mcf_vratio - np.mean(hosts_data_matched['halo_cV'])**2)/(np.var(hosts_data_matched['halo_cV']))

mcf_cnfw = mo.marked_tpcf(pos, 10**logbins, marks1=hosts_data_matched['halo_cNFW'], period=lbox,
                          normalize_by='number_counts', weight_func_id=1, num_threads=cores)
mcfn_cnfw = (mcf_cnfw - np.mean(hosts_data_matched['halo_cNFW'])**2)/(np.var(hosts_data_matched['halo_cNFW']))

mcf_ctoa = mo.marked_tpcf(pos, 10**logbins, marks1=hosts_data_matched['halo_ctoa'], period=lbox,
                          normalize_by='number_counts', weight_func_id=1, num_threads=cores)
mcfn_ctoa = (mcf_ctoa - np.mean(hosts_data_matched['halo_ctoa'])**2)/(np.var(hosts_data_matched['halo_ctoa']))

mcf_spin = mo.marked_tpcf(pos, 10**logbins, marks1=hosts_data_matched['halo_spin'], period=lbox,
                          normalize_by='number_counts', weight_func_id=1, num_threads=cores)
mcfn_spin = (mcf_spin - np.mean(hosts_data_matched['halo_spin'])**2)/(np.var(hosts_data_matched['halo_spin']))

hosts_data_matched_satflag = hosts_data_matched[np.where(hosts_data_matched['halo_satflag']==1)]

mcf_nsat = mo.marked_tpcf(pos[np.where(hosts_data_matched_satflag['halo_satflag']==1)], 10**logbins, marks1=hosts_data_matched_satflag['halo_nsat'], period=lbox, normalize_by='number_counts', weight_func_id=1, num_threads=cores)

mcfn_nsat = (mcf_nsat - np.mean(hosts_data_matched_satflag['halo_nsat'])**2)/(np.var(hosts_data_matched_satflag['halo_nsat']))
...
#########################################
# OLD ERROR CALCULATION - PLEASE IGNORE #
#########################################
#
## how we'll need to shuffle the marks N times, run the calculation N times,
## and determine the min and max range of the mark calculation. So:
...
cnfw_sort = np.sort(hosts_data_matched, order='halo_cNFW')
lowlim=int(np.floor(.2*len(cnfw_sort)))
highlim=int(np.floor(.8*len(cnfw_sort)))
low_cnfw_sort = cnfw_sort[0:lowlim]
high_cnfw_sort = cnfw_sort[highlim:-1]
...
np.savetxt(outname_match, np.transpose([10**binmids, xi, error, xi_cnfw_low, error_cnfw_low, xi_cnfw_high, error_cnfw_high,
                                 mcfn_vratio, mcfn_cnfw, mcfn_ctoa, mcfn_spin, lowererr, uppererr, mcfn_nsat, lowererr_sat,
                                 uppererr_sat]), header=r'[0]r [1]xi [2]xi_err [3]xi_cnfw_low [4]lower_err [5]xi_cnfw_high [6]high_err [7]mcf_cratio [8]mcf_cnfw [9]mcf_ctoa [10]mcf_spin [11]lower err [12]upper err [13]mcf_sat [14]lower mcf err [15]upper mcf err')
